...
export default function BrowserAgentList({ browserAgents }: { browserAgents: BrowserAgent[] }) {
	...
	return (
		<Card className="col-span-1 my-2 py-5 px-7 rounded-md">
			<div className="space-y-2 h-28">
				...
				<p className="line-clamp-4 text-muted-foreground">{browserAgent.description}</p>
			</div>
			<div className="flex space-x-2 mt-4">
				<BrowserAgentModal browserAgent={browserAgent} />
				<Link href={`/browser-agents/${browserAgent.id}`} className={buttonVariants({ size: 'sm' })}>
					Open
...
const EmptyState = () => {
  return (
		<div className="text-center">
		  <h3 className="mt-2 text-sm font-semibold text-secondary-foreground">No browsers</h3>
