...
export async function startSandbox(b: BrowserAgent) {
  if (b.sandboxId) {
    ...
    await db
      .update(browserAgent)
      .set({ sandboxActivatedAt: new Date().toISOString() })
      .where(eq(browserAgent.id, b.id));

    return { streamUrl: await sandbox.getVideoStreamUrl() };
  } else {
    // load the browser use python script
    const browserUseScript = await fs.readFile(
      process.cwd() + '/sandbox-scripts/browser_use.py',
      { encoding: 'utf-8' },
    );
    ...
    await sandbox.commands.run('pip install -U langchain-openai', {
      background: true,
    });
...
export async function stopSandbox(params: { sandboxId: string }) {
  ...
  await db
    .update(browserAgent)
    .set({ sandboxActivatedAt: null })
    .where(eq(browserAgent.sandboxId, params.sandboxId));
  return true;
}
